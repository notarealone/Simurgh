...
class ScoredDataset(Dataset):
    # MODIFIED: added chunk_size and chunk_overlap parameters
    def __init__(self, jsonl_path: str, max_documents: int = 20,
                 chunk_size: Optional[int] = None, chunk_overlap: int = 0) -> None:
        ...
        for q in order:
            ...
            for doc, score in docs_scores:
                if self.chunk_size is None or len(doc) <= self.chunk_size:
                    chunked_docs.append(doc)
                    chunked_scores.append(score)
                else:
                    # Simple character‑based overlap split
                    step = self.chunk_size - self.chunk_overlap
                    for start in range(0, len(doc), step):
                        chunk = doc[start:start + self.chunk_size]
                        if len(chunk) < 50:   # skip very short tail
                            continue
                        chunked_docs.append(chunk)
                        chunked_scores.append(score)

            # Apply max_documents limit to the chunk list (now each chunk is a "document")
            if len(chunked_docs) > max_documents:
                chunked_docs = chunked_docs[:max_documents]
                chunked_scores = chunked_scores[:max_documents]
...
def main():
    ...
    parser.add_argument("--chunk_size", type=int, default=None,
                        help="If set, split long documents into chunks of this many characters before tokenization.")
    parser.add_argument("--chunk_overlap", type=int, default=50,
                        help="Overlap in characters between adjacent chunks (only used if chunk_size is set).")
    ...
    if rank == 0:
        ...
        if args.chunk_size:
            print(f"Chunking enabled: size={args.chunk_size}, overlap={args.chunk_overlap}")
    ...
    if args.mode == "hard_neg":
        ...
        # MODIFIED: pass chunk_size and chunk_overlap
        train_ds = ScoredDataset(str(train_path), args.max_documents,
                                 chunk_size=args.chunk_size, chunk_overlap=args.chunk_overlap)
        val_ds = ScoredDataset(str(val_path), args.max_documents,
                               chunk_size=args.chunk_size, chunk_overlap=args.chunk_overlap) if val_path.exists() else None
