"""Five-arm held-out evaluation across every learner persona, sharded by ``WORLD_SIZE``.

The runner shards the ``(question, persona)`` cross product across launched workers.
``WORLD_SIZE`` defaults to 1 and is 2 only under a two-process launch. Each rank owns a
disjoint slice and writes its own append-only JSONL; rank 0 merges after a single barrier.
There is no shared mutable state, no index on disk, and no collective beyond that barrier,
which is why the process group is gloo rather than NCCL.
