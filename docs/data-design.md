...
1. Generate N=6 candidate rewrites using `PromptedRewriter` at varying temperatures
   (0.3, 0.5, 0.7, 0.9, 1.1, 1.3) to ensure diversity.
...
Splits are assigned at the **exam-file level**, not by row. Questions from the same
exam share passages (linked via `group_id`), so a row-level split would allow the
same passage to appear in both train and val — a direct leakage of the gold relevance
signal.

| Split | Exam files | Personas |
|---|---|---|
...
one `{exam_file}:{question_id}` per line.
